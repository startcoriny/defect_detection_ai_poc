...
SELECTED_DATASET_PATH = METADATA_ROOT / "v2" / "selected_dataset.csv"
INVENTORY_PATH = METADATA_ROOT / "raw_dataset_inventory.csv"
CLASS_PATH = METADATA_ROOT / "v2" / "yolo_classes.txt"
LABEL_ROOT = PROJECT_ROOT / "outputs" / "yolo_labels_v2"
DATASET_ROOT = PROJECT_ROOT / "data" / "processed" / "dataset_v2"
STAGING_ROOT = DATASET_ROOT.with_name(f"{DATASET_ROOT.name}.tmp")
LOGGER = logging.getLogger(__name__)

SPLITS = ("train", "val", "test")
DATASET_PATH = "data/processed/dataset_v2"
...
def load_selected_rows() -> list[dict[str, str]]:
    ...
    LOGGER.info("분할별 이미지 수: %s", dict(split_counts))
