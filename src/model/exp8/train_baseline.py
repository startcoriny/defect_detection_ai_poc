"""YOLO26n Baseline 모델을 GPU에서 학습하고 비교 실험 기록을 생성한다."""
...
CPU_BASELINE_SECONDS = 2 * 3600 + 10 * 60 + 53
METRIC_COLUMNS = {
...
def get_total_ram() -> str:
    """운영체제별 표준 API로 전체 물리 메모리 크기를 조회한다."""
    try:
...
def capture_environment_text() -> str:
    """실행 시점의 시스템·GPU·주요 패키지 정보를 텍스트로 구성한다."""
    ...
    lines = [
        ...
        "",
        "Compute Device",
        f"  CUDA Available : {'Yes' if cuda_available else 'No'}",
    ]
    if cuda_available:
        ...
        lines.extend(
            [
                ...
                f"  VRAM           : {format_bytes(properties.total_memory)}",
            ]
        )
    else:
...
def prepare_experiment_files(project_root: Path, experiment_dir: Path) -> None:
    """실험 폴더를 만들고 실행 환경과 데이터 요약을 기록한다."""
...
def build_experiment_data(
    *,
    ...
    return {
        ...
        "purpose": {
            "problem": (
                "EXP-P1-DET-005와 동일한 dataset_v3, YOLO26n, epochs=50 "
                "설정으로 GPU에서 재학습해 CPU 대비 학습 속도를 비교한다."
            ),
            "hypothesis": (
                "device 외 모든 설정이 동일할 때 GPU 학습이 CPU 학습 "
                "(EXP-P1-DET-005)보다 유의미하게 빠르게 완료된다."
            ),
            "success_criteria": (
                "GPU 학습이 정상 종료되고 총 실행 시간이 기록되어 "
                "EXP-P1-DET-005와 비교된다."
            ),
        },
        "dataset": {
...
def write_experiment_markdown(
    *,
    ...
    result_rows = "\n".join(
        (f"| {label} | {formatter(best_row, key)} | {formatter(last_row, key)} |")
...
- 사전 학습 사용 여부: 예
- 클래스 수: {len(classes)}
- 모델 파일 경로: {project_root / "yolo26n.pt"}
...
| 실험 | Device | 총 실행 시간 | 초 |
| --- | --- | ---: | ---: |
| EXP-P1-DET-005 | cpu | {format_duration(CPU_BASELINE_SECONDS)} | {CPU_BASELINE_SECONDS} |
| {EXPERIMENT_ID} | 0 | {format_duration(elapsed_seconds)} | {elapsed_seconds:.2f} |

- CPU 시간(초) / GPU 시간(초): {CPU_BASELINE_SECONDS / elapsed_seconds:.2f}배
...
def main() -> None:
    ...
    try:
        if not torch.cuda.is_available():
            raise RuntimeError(
                "GPU를 찾을 수 없습니다. 이 실험을 CPU로 대체 실행하면 "
                "CPU Baseline과의 비교 목적이 무효화됩니다."
            )
        prepare_experiment_files(project_root, experiment_dir)
        model_path = require_file(project_root / "yolo26n.pt")
