...
def write_results(candidates: list[dict[str, Any]]) -> None:
    ...
    with (OUTPUT_ROOT / "selected_dataset.csv").open(
    ...
    with (OUTPUT_ROOT / "included_files.txt").open(
        "w", encoding="utf-8", newline=""
    ) as included_file:
    ...
    with (OUTPUT_ROOT / "excluded_files.txt").open(
        "w", encoding="utf-8", newline=""
    ) as excluded_file:
