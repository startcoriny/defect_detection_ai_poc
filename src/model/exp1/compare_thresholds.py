...
def compare_thresholds(project_root: Path) -> list[dict[str, Any]]:
    ...
    data_path = require_file(
        (project_root / "data" / "processed" / "dataset_v1" / "data.yaml").resolve()
    )
    image_paths = load_test_images(
        project_root / "data" / "processed" / "dataset_v1" / "images" / "test"
    )
    metrics_project = (
        project_root / "outputs" / "threshold-comparison" / "metrics"
    ).resolve()
    prediction_project = (
        project_root / "outputs" / "threshold-comparison" / "images"
    ).resolve()
