...
def compare_thresholds(project_root: Path) -> list[dict[str, Any]]:
    ...
    data_path = require_file(
        (project_root / "data" / "processed" / "dataset_v3" / "data.yaml").resolve()
    )
    image_paths = load_test_images(
        project_root / "data" / "processed" / "dataset_v3" / "images" / "test"
    )
    metrics_project = (
        project_root / "outputs" / EXPERIMENT_ID / "threshold-selection" / "metrics"
    ).resolve()
    prediction_project = (
        project_root / "outputs" / EXPERIMENT_ID / "threshold-selection" / "images"
    ).resolve()
