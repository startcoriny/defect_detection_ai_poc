...
- 요구사항을 정리합니다.
- 구현 명세를 작성합니다.
- 구현에 필요한 작업 지시서를 `agent_work/prompt.md`에 준비합니다.
- 사용자가 지시서를 검토하고 승인하면, `codex exec` 명령으로 CODEX를 직접 실행하여 작업을 전달합니다. (자세한 내용은 `docs/onboarding/ai-orchestration.md`의 "CODEX 실행 방식" 참조)
