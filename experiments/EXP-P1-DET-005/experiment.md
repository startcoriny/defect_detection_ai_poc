...
**차기 실험 제안**: dataset_v3·imgsz=960을 기준으로 유지한다. `docs/12_next_experiment_plan.md`의 후속 우선순위 2번(CLAHE 대비 강조 증강)·3번(모델 크기 확대)을 다음 변수로 검토하거나, 이번에 확인된 Precision 하락을 감안해 Confidence Threshold 재선정 실험을 먼저 진행하는 것도 고려할 수 있다. 상세 계획은 필요 시 별도 문서로 작성한다.
