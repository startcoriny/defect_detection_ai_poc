"""YOLO26n Baseline 모델을 학습하고 정식 실험 기록을 생성한다."""
...
EXPERIMENT_NAME = "RT_AL_YOLO26N_960_BoxGain15"
METRIC_COLUMNS = {
...
def prepare_experiment_files(project_root: Path, experiment_dir: Path) -> None:
    ...
    shutil.copy2(
        require_file(project_root / "reports" / "dataset" / "split_distribution.csv"),
...
def load_dataset_classes(project_root: Path) -> dict[int, str]:
    ...
    summary_path = require_file(
        project_root / "reports" / "dataset" / "split_distribution.csv"
    )
...
def main() -> None:
    ...
    try:
        ...
        data_path = require_file(
            project_root / "data" / "processed" / "dataset_v1" / "data.yaml"
        )
