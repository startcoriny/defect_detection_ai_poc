...
EXPERIMENT_NAME = "RT_AL_YOLO26N_1280_ImgszSmallObject"
METRIC_COLUMNS = {
...
def build_experiment_data(
    *,
    ...
    return {
        ...
        "purpose": {
            "problem": (
                "EXP-P1-DET-008과 동일 설정(GPU, dataset_v3, YOLO26n, "
                "epochs=50 등)에서 imgsz만 960에서 1280으로 키워 Small "
                "객체 Recall 개선 여부를 검증한다."
            ),
            "hypothesis": (
                "imgsz 외 모든 설정이 동일할 때 imgsz 1280이 960보다 "
                "Small 객체(상대 면적 1% 미만) Recall을 유의미하게 개선한다."
            ),
            "success_criteria": (
                "GPU 학습이 정상 종료되고 후속 Test 추론과 크기별 평가에서 "
                "EXP-P1-DET-008 대비 Small 객체 Recall을 비교할 수 있다."
            ),
        },
        "dataset": {
    ...
    try:
        if not torch.cuda.is_available():
            raise RuntimeError(
                ...
                "EXP-P1-DET-008과의 imgsz 비교 목적이 무효화됩니다."
            )
