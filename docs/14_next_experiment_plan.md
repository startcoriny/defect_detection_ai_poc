...
`docs/13_next_experiment_plan.md`에서 보류했던 마지막 후보인 **모델 크기 확대**를 이번 실험의 변수로 채택한다. 가설: yolo26n(2.5M 파라미터)은 용량이 작아 결함 경계의 미세한 형태를 정밀하게 회귀하기에 부족할 수 있다. yolo26s(10.0M 파라미터, 약 4배)로 용량을 키우면 박스 위치·크기 정밀도가 개선될 것이다.
