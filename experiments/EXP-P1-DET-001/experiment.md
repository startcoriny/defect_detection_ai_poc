...
작업24에서 오탐(3건)·미탐(48건)·위치 오류(2건)·클래스 오류(0건) 전 사례를 대표 검토했다. 상세 내용은 `docs/reports/08_error_analysis.md` 참조.
...
작업17~24에서 확인한 근거(83% 미탐, Small Recall 0.121 < Medium 0.167, 저대비 결함 위주 미탐)를 종합해 "작은/저대비 결함 Recall 부족"을 Baseline의 가장 큰 문제로 선정했다. 다음 실험(EXP-P1-DET-002)은 `imgsz`만 640→960으로 변경하고 나머지 조건은 전부 동일하게 유지한다. 상세 근거·성공 기준·후속 우선순위는 `docs/decisions/10_next_experiment_plan.md` 참조.
