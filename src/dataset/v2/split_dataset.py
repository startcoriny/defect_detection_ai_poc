...
REPORT_ROOT = PROJECT_ROOT / "reports" / "dataset" / "v2"
...
def load_annotations(
    ...
    _, rows = read_csv(METADATA_ROOT / "v2" / "bbox_annotations.csv")
    annotations = []
    for row in rows:
...
def build_split_assignments(
    ...
    LOGGER.info("층화 키별 이미지 수: %s", actual_counts)
...
def write_selected_dataset(
    ...
    path = METADATA_ROOT / "v2" / "selected_dataset.csv"
...
def write_validation_report(
    ...
    strata = sorted(set(image_strata.values()))
    lines = [
        "# 데이터셋 분할 검증 보고서",
        "",
        "## 분할 결과",
...
def main() -> int:
    ...
    field_names, rows = read_csv(METADATA_ROOT / "v2" / "selected_dataset.csv")
